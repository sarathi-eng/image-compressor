  ...
  return (
    <main>
      ...
      <article style={{ padding: '4rem 2rem', maxWidth: '800px', margin: '0 auto' }}>
        ...
        {post.aeoAnswer && (
          <section style={{ marginBottom: '3rem', padding: '2rem', background: '#0070f322', borderRadius: '12px', border: '1px solid #0070f3' }}>
            <h2 style={{ fontSize: '1.2rem', marginTop: 0, color: '#0070f3' }}>Quick Answer</h2>
            <p style={{ fontSize: '1.1rem', margin: 0, color: '#fff' }}>{post.aeoAnswer}</p>
          </section>
        )}
          ...
          <section style={{ marginTop: '4rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
              {post.faqs.map((faq, index) => (
                <div key={index}>
                  <h3 style={{ fontSize: '1.3rem', marginBottom: '0.5rem', color: '#fff' }}>{faq.question}</h3>
                  <p style={{ color: '#aaa', margin: 0 }}>{faq.answer}</p>
                </div>
              ))}
            </div>
          </section>
